...
- Approval lives on the bottom row (keys 12–15), added at the user's request 2026-08-09.
  `approve`, `reject` and `interrupt` require a 300 ms hold (reject joined 2026-08-29 at
  the user's request); `next_attention` is instant because it sends no keystrokes and
  cannot approve anything. Guards are enforced in `ActionRouter.run_action` and
